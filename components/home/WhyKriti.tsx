        ...
        <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="dots" x="0" y="0" width="24" height="24" patternUnits="userSpaceOnUse">
              <circle cx="2" cy="2" r="1" fill="#166534" />
            </pattern>
          </defs>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {REASONS.map((reason, i) => {
            ...
            return (
              <div
                key={i}
                className="bg-brand-navy-light rounded-xl p-7 border border-brand-gray-dark/10 hover:border-brand-blue/40 transition-colors duration-300"
              >
                <div className="w-11 h-11 bg-brand-blue/15 rounded-lg flex items-center justify-center mb-5">
                  <Icon className="w-5 h-5 text-brand-blue-light" />
                </div>
                <h3 className="font-display font-bold text-brand-gray-dark text-lg mb-3">
                  {reason.title}
                </h3>
